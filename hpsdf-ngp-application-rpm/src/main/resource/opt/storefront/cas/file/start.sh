...
if [ "$user" != "sdpadmin" ] ; then
        echo "only sdpadmin user have permission to run it."
        exit
fi
if [ "$pids" = "" ] ; then
        ...
        nohup /opt/storefront/jboss/bin/run.sh -c all -g storefrontgroup -b 0.0.0.0 -Djavax.net.ssl.trustStore="/opt/storefront/cas/scripts/server.keystore" > /dev/null &
