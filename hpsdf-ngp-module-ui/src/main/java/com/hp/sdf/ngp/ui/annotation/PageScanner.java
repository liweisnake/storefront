 ...
 * of {@Link Page} in order to mount it using its class simple name as
 * URL
 * 
 * 
 */
public class PageScanner extends AnnotatedMountScanner {
	...
	public List<Class<?>> getPatternMatches(String pattern) {
		MatchingClassResources resources = new MatchingClassResources(pattern);
		List<Class<?>> mountPaths = resources.getClassMatches(Page.class);
		return mountPaths;
	}
