...
public class ViewAssetDetailPanel extends BreadCrumbPanel {

	private final static Log log = LogFactory.getLog(ViewAssetDetailPanel.class);
	...
	public ViewAssetDetailPanel(String id, IBreadCrumbModel breadCrumbModel, Long appId, BreadCrumbPanel caller) {
	...
	private void populate(Long appId) {
		...
		RatingPanel ratingPanel = new CustomizedRatingPanel("rating", null, 5, false) {
			...
			protected void onRated(int rating, AjaxRequestTarget target) {
				...
				Double avgRank = applicationService.saveOrUpdateAssetRating(ViewAssetDetailPanel.this.appId, userId, new Double(rating - 1));
				log.debug("avg rating=" + avgRank.intValue());
				app.setAverageUserRating(avgRank);

				// update average rating
				ViewAssetDetailPanel.this.remove("avgrating");
				if (avgRank == null || avgRank.isNaN()) {
					avgRank = 0.0D;
				}
				DecimalFormat ratingFormat = new DecimalFormat("#0.0");
				Label avgRatingLabel = new Label("avgrating", ratingFormat.format(avgRank + 1));
				ViewAssetDetailPanel.this.add(avgRatingLabel);

				// update my rating
				ViewAssetDetailPanel.this.remove("myrating");
				myRankLabel = new Label("myrating", Integer.toString(rating));
				ViewAssetDetailPanel.this.add(myRankLabel);
			}
		...
		DecimalFormat ratingFormat = new DecimalFormat("#0.0");
		Label avgRatingLabel = new Label("avgrating", ratingFormat.format(avgRating + 1));
		add(avgRatingLabel);

		// add my rating
		String userId = WicketSession.get().getUserId();
		String myRank = this.getLocalizer().getString("default.rating", this);
		if (userId != null) {
			AssetRating myrating = applicationService.getAssetRating(userId, appId);
			if (myrating != null) {
				myRank = new Long((long) (myrating.getRating().doubleValue() + 1)).toString();
			}
		}
		myRankLabel = new Label("myrating", myRank);
		add(myRankLabel);
		myRankLabel.setOutputMarkupId(true);
